...
class MatchPredictor:
    ...
    def _build_features_for_matchup(self, home_team, away_team):
        ...
        home_form = self.engineer._calculate_team_form(
            home_team, current_date, 'home')
        away_form = self.engineer._calculate_team_form(
            away_team, current_date, 'away')
        home_overall = self.engineer._calculate_team_form(
    ...
    def predict_match(self, home_team, away_team):
        ...
        for threshold in ['05', '15', '25']:
            ...
            raw_probs[threshold] = float(prob_over)

        # Enforce monotonic decreasing probabilities
        # Over 0.5 >= Over 1.5 >= Over 2.5
        adjusted_probs = {}
        adjusted_probs['05'] = raw_probs['05']
        # Cannot exceed Over 0.5
        adjusted_probs['15'] = min(raw_probs['15'], adjusted_probs['05'])
        # Cannot exceed Over 1.5
        adjusted_probs['25'] = min(raw_probs['25'], adjusted_probs['15'])

        # Build predictions with adjusted probabilities
        for threshold in ['05', '15', '25']:
            prob_over = adjusted_probs[threshold]
            ...
            predictions['goals_match'][f'over_{threshold}'] = {
                ...
                'over_odds': round(1 / prob_over, 2) if prob_over > 0.01 else 999,
        ...
        for threshold in ['05', '15', '25']:
            ...
            raw_home_probs[threshold] = float(prob_home_over)

        # Enforce monotonic decreasing for home team
        adjusted_home_probs = {}
        adjusted_home_probs['05'] = raw_home_probs['05']
        adjusted_home_probs['15'] = min(
            raw_home_probs['15'], adjusted_home_probs['05'])
        adjusted_home_probs['25'] = min(
            raw_home_probs['25'], adjusted_home_probs['15'])

        # Build home team predictions
        for threshold in ['05', '15', '25']:
            prob_home_over = adjusted_home_probs[threshold]
            threshold_val = float(threshold) / 10
        ...
        adjusted_away_probs = {}
        adjusted_away_probs['05'] = raw_away_probs['05']
        adjusted_away_probs['15'] = min(
            raw_away_probs['15'], adjusted_away_probs['05'])
        adjusted_away_probs['25'] = min(
            raw_away_probs['25'], adjusted_away_probs['15'])

        # Build away team predictions
        for threshold in ['05', '15', '25']:
            prob_away_over = adjusted_away_probs[threshold]
            threshold_val = float(threshold) / 10
            ...
            predictions['goals_team'][f'away_over_{threshold}'] = {
                ...
                'odds': round(1 / prob_away_over, 2) if prob_away_over > 0.01 else 999
            }
