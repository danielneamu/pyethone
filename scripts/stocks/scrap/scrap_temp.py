...
def main(input_csv, output_csv):
    with open(input_csv, 'r') as infile:
        ...
        rows = [row for _, row in zip(range(3), reader)]  # Read only the first 3 rows
...
if __name__ == "__main__":
    ...
    print(f"Script executed in {(end_time - start_time).total_seconds():.2f} seconds at {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("------------")
